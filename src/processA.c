...
{   
    ...
    int connection_fd, client_size,port,choice,bind_result,connect_result;
        ...
        switch(choice){
            ...
            case 2:
                    ...
                    while(TRUE){
                        ...
                        connect_result=connect(client_fd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)); //connect to the server
                        
                        if(connect_result!=-1)  // giving three attempts to connect to the server
                            break;
                            ...
                            fprintf(logfile,"\nTIME: %s Error: %s Line: %d\n",ctime(&curtime),strerror(errno),__LINE__);
                            fflush(logfile);

                            if(counter==0)
                                controller(kill(pid_A,SIGTERM),__LINE__); //close the process if counter is zero

                            printf("You have %d more attempts, then the service will close.\n\n",counter);
                            fflush(stdout);
                        }
                    }
            ...
            case 3:
                    ...
                    while(TRUE){   // giving three attempts to choose the correct port for the server
                        ...
                        // bind
                        bind_result=bind(server_fd, (struct sockaddr *)&serv_addr, sizeof(serv_addr));

                        if(bind_result!=-1)
                            break;
                        else if(bind_result==-1){
                            ...
                            if(counter==0)
                                controller(kill(pid_A,SIGTERM),__LINE__); //close the process
